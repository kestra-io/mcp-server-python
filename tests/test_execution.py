...
def _exec_id(result) -> str:
    """Extract execution ID from a tool result."""
    return json.loads(result.content[0].text).get("id", "") if result and result.content and result.content[0].text else ""


@pytest.mark.asyncio
async def test_execute_flow(kestra_client, cleanup):
    """Test executing a flow with inputs and outputs."""
    response_json = await create_flow("app_get_data_flow.yaml", kestra_client, cleanup)
    ...
    response_json = await create_flow("get_data_updated.yaml", kestra_client, cleanup)
...
@pytest.mark.asyncio
async def test_manage_executions(kestra_client, cleanup):
    """Test managing an execution."""
    subflow_response_json = await create_flow("hello_mcp.yaml", kestra_client, cleanup)
    print(subflow_response_json)
    response_json = await create_flow("sleeper.yaml", kestra_client, cleanup)
    print(response_json)
    flow_id = response_json.get("id", "")
    namespace = response_json.get("namespace", "")
...
@pytest.mark.asyncio
async def test_execute_flow_with_subflow(kestra_client, cleanup):
    ...
    cleanup.track_execution(parent_execution_id)
    correlation_id = None
    ...
    cleanup.track_execution(subflow_execution_id)
...
@pytest.mark.asyncio
async def test_delete_execution(kestra_client, cleanup):
    """Test deleting an execution."""
    response_json = await create_flow("healthcheck.yaml", kestra_client, cleanup)
    ...
    print("Execute flow response:", response_json)
    execution_id = response_json["id"]
    # Don't track this execution — the test itself deletes it

    # wait for the execution to be in a terminal state
    response_json = await poll_for_execution(
        kestra_client, execution_id, max_retries=10, retry_interval=1
    )
